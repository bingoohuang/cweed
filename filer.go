...
// FilerUploadResult upload result which response from filer server.
// According to https://github.com/chrislusf/seaweedfs/wiki/Filer-Server-API.
...
func (f *Filer) UploadFile(localFilePath, newPath, collection, ttl string) (result *FilerUploadResult, err error) {
	...
	if err == nil {
		var data []byte
		data, _, err = f.client.upload(encodeURI(*f.base, newPath, normalize(nil, collection, ttl)), localFilePath, fp.MimeType, fp.reader)
		if err == nil {
...
// Upload content.
func (f *Filer) Upload(r io.Reader, fileSize int64, newPath, collection, ttl string) (result *FilerUploadResult, err error) {
	fp := NewFilePartByReader(ioutil.NopCloser(r), newPath, fileSize)

	var data []byte
	data, _, err = f.client.upload(encodeURI(*f.base, newPath, normalize(nil, collection, ttl)), newPath, "", ioutil.NopCloser(r))
	if err == nil {
		result = &FilerUploadResult{}
		err = json.Unmarshal(data, result)
	}
