...
// Weed client containing almost features/operations to interact with SeaweedFS
type Weed struct {
	master    *url.URL
	filers    []*Filer
	chunkSize int64
...
// New create new seaweed client. Master url must be a valid uri (which includes scheme).
func New(masterURL string, filers []string, chunkSize int64, client *http.Client) (c *Weed, err error) {
...
func (c *Weed) Filers() []*Filer { return c.filers }

// Grow pre-Allocate Volumes.
func (c *Weed) Grow(count int, collection, replication, dataCenter string) error {
...
func (c *Weed) GrowArgs(args url.Values) (err error) {
	_, _, err = c.client.get(encodeURI(*c.master, "/vol/grow", args), nil)
	return
}

// Lookup volume ID.
func (c *Weed) Lookup(volID string, args url.Values) (result *LookupResult, err error) {
	return c.doLookup(volID, args)
}

func (c *Weed) doLookup(volID string, args url.Values) (result *LookupResult, err error) {
	args = normalize(args, "", "")
...
func (c *Weed) LookupServerByFileID(fileID string, args url.Values, readonly bool) (server string, err error) {
	var parts []string
	...
	l, lookupError := c.Lookup(parts[0], args)
	if lookupError != nil {
		err = lookupError
	} else if len(l.VolumeLocations) == 0 {
		err = ErrFileNotFound
	}

	if err == nil {
		if readonly {
			server = l.VolumeLocations.RandomPickForRead().PublicURL
		} else {
			server = l.VolumeLocations.Head().URL
		}
	}
...
func (c *Weed) LookupFileID(fileID string, args url.Values, readonly bool) (fullURL string, err error) {
...
func (c *Weed) GC(threshold float64) (err error) {
	args := url.Values{
...
func (c *Weed) Status() (result *SystemStatus, err error) {
...
func (c *Weed) ClusterStatus() (result *ClusterStatus, err error) {
...
func (c *Weed) Assign(args url.Values) (result *AssignResult, err error) {
...
func (c *Weed) Submit(filePath string, collection, ttl string) (result *SubmitResult, err error) {
...
func (c *Weed) SubmitFilePart(f *FilePart, args url.Values) (result *SubmitResult, err error) {
	data, _, err := c.client.upload(encodeURI(*c.master, "/submit", args), f.FileName, f.MimeType, f.reader)
	if err == nil {
		result = &SubmitResult{}
		err = json.Unmarshal(data, result)
	}
...
func (c *Weed) Upload(fileReader io.Reader, fileName string, size int64, collection, ttl string) (fp *FilePart, err error) {
	fp = NewFilePartByReader(ioutil.NopCloser(fileReader), fileName, size)
...
func (c *Weed) UploadFile(filePath string, collection, ttl string) (cm *ChunkManifest, fp *FilePart, err error) {
	fp, err = NewFilePart(filePath)
	if err == nil {
		fp.Collection, fp.TTL = collection, ttl
...
func (c *Weed) UploadFilePart(f *FilePart) (cm *ChunkManifest, err error) {
	collectionTTL := normalize(nil, f.Collection, f.TTL)
	searchCollectionTTl := normalize(nil, f.Collection, "")

	if f.FileID == "" {
		res, err := c.Assign(collectionTTL)
		if err != nil {
			return nil, err
		}
	...
	if f.Server == "" {
		if f.Server, err = c.LookupServerByFileID(f.FileID, searchCollectionTTl, false); err != nil {
			return
		}
	}
	...
	if c.chunkSize > 0 && f.FileSize > c.chunkSize {
		...
		for i := int64(0); i < chunks; i++ {
			...
			if e != nil { // delete all uploaded chunks
				_ = c.DeleteChunks(cm, searchCollectionTTl)
				return nil, e
			}
		...
		_, _, err = c.client.upload(encodeURI(base, f.FileID, args), baseName, f.MimeType, f.reader)
	}
...
func (c *Weed) BatchUploadFiles(files []string, collection, ttl string) (results []*SubmitResult, err error) {
...
func (c *Weed) BatchUploadFileParts(files []*FilePart, collection, ttl string) ([]*SubmitResult, error) {
...
func (c *Weed) uploadTask(file *FilePart) *workerpool.Task {
...
func (c *Weed) Replace(fileID, fileName string, newContent io.Reader, size int64, collection, ttl string, deleteFirst bool) (err error) {
	fp := NewFilePartByReader(ioutil.NopCloser(newContent), fileName, size)
	fp.Collection, fp.TTL = collection, ttl
	fp.FileID = fileID
...
func (c *Weed) ReplaceFile(fileID, localFilePath string, deleteFirst bool) (err error) {
...
func (c *Weed) ReplaceFilePart(f *FilePart, deleteFirst bool) (err error) {
...
func (c *Weed) uploadChunk(f *FilePart, filename string) (result *AssignResult, fileID string, size int64, err error) {
	// Assign first to get file id and url for uploading
	result, err = c.Assign(normalize(nil, f.Collection, f.TTL))
	if err == nil {
		fileID = result.FileID

		base := *c.master
		base.Host = result.URL

		// do upload
		var v []byte
		v, _, err = c.client.upload(encodeURI(base, result.FileID, nil), filename,
			"application/octet-stream", io.LimitReader(f.reader, c.chunkSize))
		if err == nil {
...
func (c *Weed) uploadManifest(f *FilePart, manifest *ChunkManifest) (err error) {
	...
	if err == nil {
		...
		_, _, err = c.client.upload(encodeURI(base, f.FileID, args), manifest.Name,
			"application/json", bufReader)
	}
	return
}

// Download file by id.
func (c *Weed) Download(fileID string, args url.Values, callback func(io.Reader) error) (fileName string, err error) {
...
func (c *Weed) DeleteChunks(cm *ChunkManifest, args url.Values) (err error) {
...
func (c *Weed) deleteFileTask(fileID string, args url.Values) *workerpool.Task {
...
// DeleteFile by id.
func (c *Weed) DeleteFile(fileID string, args url.Values) (err error) {
