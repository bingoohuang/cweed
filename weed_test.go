...
func init() {
	masterURL := os.Getenv("WEED_MASTER_URL")
	if masterURL == "" {
		masterURL = "http://127.0.0.1:9333"
	}

	var filer []string
	if v := os.Getenv("WEED_FILER_URL"); v != "" {
		filer = []string{v}
	}
	sw, _ = New(masterURL, filer, 8096, &http.Client{Timeout: 5 * time.Minute})

	MediumFile = os.Getenv("WEED_MEDIUM_FILE")
	if MediumFile == "" {
		MediumFile = "/Users/bingoobjca/Downloads/500px/stock-photo-1028038592.jpg"
	}
	SmallFile = os.Getenv("WEED_SMALL_FILE")
	if SmallFile == "" {
		SmallFile = "/Users/bingoobjca/Downloads/500px/stock-photo-1028001550.jpg"
	}
}
...
func TestUploadLookupserverReplaceDeleteFile(t *testing.T) {
	for i := 0; i < 2; i++ {
		...
		require.Nil(t, fiErr)
		size = fi.Size()
		require.Nil(t, sw.Replace(fp.FileID, "ta.txt", fs, size, "", "", false))
		require.Nil(t, sw.DeleteFile(fp.FileID, nil))
		fs.Close()
	}
}
